...
class TrainingConfig:
    ...
    best_model_eval_metric_weight: float = 1.0
    best_model_train_loss_weight: float = 0.0
...
class Config:
    ...
    def validate(self) -> None:
        ...
        # Best-model weighted score validation
        if self.training.best_model_eval_metric_weight < 0 or self.training.best_model_train_loss_weight < 0:
            raise ValueError("best_model_*_weight values must be >= 0")
        if self.training.best_model_eval_metric_weight == 0 and self.training.best_model_train_loss_weight == 0:
            raise ValueError("At least one of best_model_eval_metric_weight or best_model_train_loss_weight must be > 0")
