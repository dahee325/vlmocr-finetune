...
class Tokenizer(PipelineStep):
    ...
    def __call__(self, sample: Sample) -> Sample:
        ...
        inputs = self.processor(
            ...
            images=[main_image],
            padding=True,
            return_tensors="pt", # np가 원값임.
        )

        # Get labels by tokenizing the output text
        labels = self.processor(text=[response], padding=True, return_tensors="pt")  # np가 원값임.
        ...
        # end_tokens = np.array(end_tokens, dtype=inputs.input_ids.dtype) # 원래 코드
        end_tokens = np.array(end_tokens, dtype=inputs.input_ids.numpy().dtype)
